...
final dashboardMetricsProvider = StateNotifierProvider<DashboardMetricsNotifier, DashboardMetrics>((ref) {
  return DashboardMetricsNotifier(ref);
});

class DashboardMetricsNotifier extends StateNotifier<DashboardMetrics> {
  final Ref _ref;

  DashboardMetricsNotifier(this._ref) : super(const DashboardMetrics(
    activeOrders: 0,
    occupiedTables: 0,
    totalTables: 15,
    revenue: 0,
  ...
  void _updateMetrics() {
    final orders = _ref.read(liveOrdersProvider);
    final active = orders
  ...
  Future<void> refresh() async {
    await _ref.read(liveOrdersProvider.notifier).refresh();
    _updateMetrics();
  }
}
