...
class FcmService {
  ...
  /// Receives only `message.data` so main.dart never has to import
  /// firebase_messaging (web build can't link its bindings).
  ...
  Future<void> init(String? authToken) async {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((message) {
      onMessageOpened?.call(Map<String, dynamic>.from(message.data));
    });
    ...
    if (initial != null) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        onMessageOpened?.call(Map<String, dynamic>.from(initial.data));
      });
    }
  }
